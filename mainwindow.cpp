    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->actionCount_money_for_fond, SIGNAL(triggered()), this, SLOT(fondNameCount()));
}
...
void MainWindow::fondNameCount()
{
    fondName = new FondNameDialog(queue);
    fondName->show();
}
...
{
    if(ui->radioButton->isChecked()) ui->label->setText("Add buy:"),
            ui->label_5->setVisible(false), ui->lineEdit_2->setVisible(false);
    else  ui->label->setText("Add donation:"), ui->label_5->setVisible(true),
            ui->lineEdit_2->setVisible(true);
    if(queue->isEmpty()) ui->actionShow->setDisabled(true),
            ui->actionSave_as->setDisabled(true),
            ui->actionCount_money_for_fond->setDisabled(true),
            ui->actionCount_money->setDisabled(true),
            ui->actionDelete_first->setDisabled(true);

    else ui->actionShow->setEnabled(true), ui->actionSave_as->setEnabled(true),
    ui->actionSave_as->setEnabled(true),
    ui->actionCount_money_for_fond->setEnabled(true),
    ui->actionCount_money->setEnabled(true),
    ui->actionDelete_first->setEnabled(true);
}
...
void MainWindow::push()
{
    ...
    {
        queue->push(new Donation(Time(_time.hour(), _time.minute(), _time.second()),
                                 Date(_date.day(), _date.month(), _date.year()),
                                 ui->lineEdit->text().toInt(), ui->lineEdit_2->text()));
    }
    else
    {
        queue->push(new Buy(Time(_time.hour(), _time.minute(), _time.second()),
                            Date(_date.day(), _date.month(), _date.year()),
