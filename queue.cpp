...
int Queue::countFond(QString fond_name)
{
    int sum = 0;
    for(Node* temp = first; temp; temp = temp->next)
    {
        ...
        {
            if(dynamic_cast<Donation*>(temp->data)->getName()==fond_name) sum+=temp->data->getPrice();
        }
    }
    return sum;
}
...
void Queue::save(QString f_name)
{
    ...
    for(Node* temp = first; temp; temp=temp->next)
    {
            ...
            stream<<QString::number(temp->data->getDate().day)+"/"+QString::number(temp->data->getDate().month)+"/"
                    +QString::number(temp->data->getDate().year)<<QString::number(temp->data->getTime().hours)+":"
                    +QString::number(temp->data->getTime().minutes)+":"
                    +QString::number(temp->data->getTime().seconds)<<QString::number(temp->data->getPrice());
...
void Queue::load(QString f_name)
{
    ...
    while(!file.atEnd())
    {
        ...
        if(_type.toInt()==0)
            push(new Buy(Time(_t.at(0).toInt(),_t.at(1).toInt(),
                              _t.at(2).toInt()),Date(_d.at(0).toInt(),
                                                     _d.at(1).toInt(),
                                                     _d.at(2).toInt()),
                                                     _price.toInt()));
        else
        {
            QString _name;
            stream>>_name;
            push(new Donation(Time(_t.at(0).toInt(),_t.at(1).toInt(),
                                   _t.at(2).toInt()),Date(_d.at(0).toInt(),
                                                          _d.at(1).toInt(),
                                                          _d.at(2).toInt()),
                                                          _price.toInt(), _name));
